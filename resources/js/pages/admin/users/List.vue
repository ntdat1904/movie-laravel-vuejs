        ...
        <div class="portlet-body">
            <div class="table-toolbar">
                ...
                <b-col md="12" class="text-right">
                    ...
                    <button class="btn bold yellow border-global" style="margin: 10px 15px 0 0" @click="getUsers">
                        Search
                    </button>
                </b-col>
            </div>
            <b-col md="12">
                ...
                <b-col md="6" class="my-1  text-right">
                    <div class="btn-group">
                        <router-link :to="{name: 'admin.user.create'}" class="nav-link">
            ...
            >
                ...
                <template slot="role" slot-scope="data">
                    <span class="label label-sm label-success" v-if="data.value == 1"> User </span>
                    <span class="label label-sm label-warning" v-if="data.value == 2"> Admin </span>
                </template>
                <template slot="actions" slot-scope="data">
                    <div class="actions">
                        <router-link
                            :to="{ name: 'admin.user.detail', params: { id: data.item.id }}"
                            class="btn btn-circle btn-icon-only btn-default">
                            <i class="icon-eye"></i>
                        </router-link>
                        <router-link
                            :to="{ name: 'admin.user.edit', params: { id: data.item.id }}"
                            class="btn btn-circle btn-icon-only btn-default">
    ...
    export default {
        ...
        data() {
            return {
                ...
                idModal: 0,
                typeModal: "",
                options: ['ID', 'Name', 'Role', 'Email', 'Create At'],
                fields: [
                    {
                        key: 'checkbox',
                        label: '',
                ...
                arrayField: [
                    ...
                    },
                    {
                        ...
                        status: false,
                        html: '<div role="group" class="input-group" style="width: 50vh">' +
                            '<input type="text" id="search_email" placeholder="Email" ' +
                            'class="form-control input-sm input-small input-inline" style="width: 100% !important; border-radius: 5px !important">' +
                            '</div>'
                    },
                    {
                        ...
                        status: false,
                        html: '<div role="group" class="input-group" style="width: 50vh">' +
                            '<select id = "search_role" class = "form-control input-sm input-small input-inline" style = "width: 100% !important; border-radius: 5px !important" >\n' +
                ...
                paginate: {
                    ...
                    from: 1,
                    last_page: 3,
                    path: "http://movie.test/api/auth/user",
                    prev_page_url: null,
                    to: 1,
                    page: 1,
                }
        ...
        methods: {
            ...
            getUsers() {
                    ...
                    for (const key in this.dataSearch) {
                        switch (key) {
                            ...
                            case "name":
                                ...
                                break;
                            case "email":
                                this.arrayField.filter(f => f.key === 'Email')[0].status = true;
                                document.getElementById('search_' + key).value = this.dataSearch[key];
                                break;
                            case "role":
                                this.arrayField.filter(f => f.key === 'Role')[0].status = true;
                                document.getElementById('search_' + key).value = this.dataSearch[key];
                                break;
                            case "from":
                ...
                this.axios.get('auth/users', {params: query})
                    .then((res) => {
                        let data = [];
                        data = res.data.users;
            ...
            selectAllchange() {
                if (this.arrId.length === this.users.length) {
                    this.arrId = [];
                }
            },
            ...
            removeFilter(field) {
                ...
                switch (field) {
                    ...
                    case "Name":
                        ...
                        break;
                    case "Email":
                        this.dataSearch['email'] = '';
                        break;
                    case "Role":
                        this.dataSearch['role'] = '';
                        break;
                    case "Create At":
                        this.dataSearch['to'] = '';
        ...
        computed: {
            selectAll: {
                get: function () {
                    return this.users ? this.arrId.length === this.users.length : false;
                },
                set: function (value) {
                    var selected = [];
